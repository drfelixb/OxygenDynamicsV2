function T=compareSurgeContinuityReference(previousRoot,newRoot,outputRoot)
...
for session=["M400-01-baseline-awake","M401-01-baseline-awake","FB2411","FB2312-baseline-awake"]
 ...
 for name=cases
  old=fullfile(previousRoot,session,name);
  assert(strcmp(oxygenFileSHA256(fullfile(old,'challenge_original.tif')),oxygenFileSHA256(fullfile(newRoot,session,name,'challenge_original.tif'))),'Challenge inputs differ.');
...
f=fopen(fullfile(outputRoot,'comparison-verification.json'),'w');assert(f>=0);fprintf(f,'%s\n',jsonencode(struct('SameInputCases',20,'SinkNativeMasksUnchanged',true,'SinkTimingAndIdentitiesUnchanged',true,'SinkAmplitudeMayChangeThroughCrossSignBaselineExclusion',true),'PrettyPrint',true));fclose(f);
end
