function Results=runSurgeTrackingValidation(referenceRoot,outputRoot)
...
Sessions=["M400-01-baseline-awake","M401-01-baseline-awake","FB2411","FB2312-baseline-awake"];
Cases=["control","smooth_pairs","single_clean","single_noise","moving_clean"];
Windows=[41 60;66 85;111 130;146 165;196 215;246 265;101 160;101 160];
...
for r=1:numel(Sessions)
 ...
 for ci=1:numel(Cases)
  name=Cases(ci);fraction=zeros(2,N);truth=1:8;
  if name=="smooth_pairs",truth=1:6;elseif name=="moving_clean",truth=8;elseif name~="control",truth=7;end
  if name~="control"
   for k=1:2
    polarity=2*k-3;
    for q=truth
  ...
  centerX=repmat(centers(:,1),1,N);centerY=repmat(centers(:,2),1,N);
  if name=="moving_clean"
   t=101:160;centerX(:,t)=round(centers(:,1)+4.75*(t-101)/P.PixelSize);
  end
  assert(all(centerX-radius>=1 & centerX+radius<=W & centerY-radius>=1 & centerY+radius<=H,'all'));
  ...
  for t=1:N
   ...
   for k=1:2
    mask=(xx-centerX(k,t)).^2+(yy-centerY(k,t)).^2<=radius^2;
  ...
  M=struct('SourceTiff',source,'SourceSHA256',saved.Conversion.TiffSHA256,'SourceProfile',P, ...
   ...
   'RadiusPixels',radius,'CenterXBySignAndFrame',centerX,'CenterYBySignAndFrame',centerY,'MotionUmPerSec',4.75,'SampleHz',P.SampleHz,'Windows',Windows,'EvaluatedTruthRows',truth, ...
   ...
   'Interpretation','Both signs injected at separate sites; moving_clean translates each patch 4.75 um/sec during its support. Control has no injections. Single-noise adds temporally independent, within-patch correlated Gaussian intensity perturbations to the latent pulse; not a photon-noise model.');
  ...
  for k=1:2
   ...
   eligiblePixels=S.AnalysisInfo.SinkEligibleTissuePixels;
   if k==2,eligiblePixels=U.AnalysisInfo.SurgeEligibleTissuePixels;end
   eligibleMask=false(H,W);eligibleMask(eligiblePixels)=true;
   for q=truth
    a=Windows(q,1);b=Windows(q,2);truthMasks=cell(1,N);truthVolume=0;eligibleVolume=0;
    for t=a:b
     cx=centers(k,1);if q==8,cx=round(cx+4.75*(t-101)/P.PixelSize);end
     mask=(xx-cx).^2+(yy-centers(k,2)).^2<=radius^2;
     truthMasks{t}=mask;truthVolume=truthVolume+nnz(mask);eligibleVolume=eligibleVolume+nnz(mask&eligibleMask);
    end
    best=0;row=0;hits=0;nativeA=NaN;nativeB=NaN;
    for e=1:height(E)
     ...
     for t=reshape(frames,1,[])
      volume=volume+numel(pc{t});if t>=a&&t<=b,intersection=intersection+nnz(truthMasks{t}(pc{t}));end
     end
     hits=hits+(intersection>0);iou=intersection/(volume+truthVolume-intersection);
    ...
    R.TruthEligibleFraction=eligibleVolume/truthVolume;
    Results=[Results;R]; %#ok<AGROW>
   end
  end
  writetable(Results,fullfile(outputRoot,'tracking-signal-results.csv'));writetable(QCAll,fullfile(outputRoot,'recording-qc.csv'));
...
save(fullfile(outputRoot,'tracking-validation.mat'),'Results','QCAll','Audits');
end
