...
The saved contract is currently `3.0-dev`, detector `existing-v2-retain-close-terminal-3`, measurement `event-footprint-sign-qc-4`, statistics `mouse-strict-sign-qc-5`. These are development identities, not a frozen publication release. Rule changes require contract updates. The current identity is manually maintained; it is not a source-code checksum.
