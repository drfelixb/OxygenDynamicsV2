...
Detection-mask bounds and measurement bounds are different quantities. Sink tables retain `NativeStartFrame/NativeEndFrame`; native-mask exports also label `DetectionStartFrame/DetectionEndFrame`. `StartFrame/EndFrame` describe the locally refined or retained native measurement window. Native area exists on detection frames. See [event-local timing](LOCAL_SINK_TIMING.md) for limits and unresolved-boundary flags.

## Time and area

For N frames at fs Hz, recording duration is N/fs seconds. MATLAB frame f represents the interval [(f-1)/fs, f/fs). An event from frame a through b has duration (b-a+1)/fs. The new sink timing rule keeps measurement windows at one site disjoint; events at different sites may overlap. The site raster remains Boolean.
...
The saved contract is currently `3.0-dev`, detector `existing-v2-surge-physical-adjacent-4`, measurement `event-footprint-surge-tracking-qc-5`, statistics `mouse-strict-tracking-qc-6`. These are development identities, not a frozen publication release. Rule changes require contract updates. The current identity is manually maintained; it is not a source-code checksum.
